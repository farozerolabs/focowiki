...
| `WORKER_SOURCE_FILE_CONCURRENCY` | 是 | worker service 同时处理的 source-file job 数。 |
| `WORKER_CLAIM_BATCH_SIZE` | 是 | 单次 worker 轮询最多 claim 的排队 job 数。 |
| `WORKER_POLL_INTERVAL_MS` | 是 | 没有排队 job 时的 worker 轮询间隔。 |
| `WORKER_LOCK_TTL_SECONDS` | 是 | worker 崩溃后运行中 job 可以被重新 claim 的等待时间。 |
| `WORKER_HEARTBEAT_INTERVAL_MS` | 是 | 更新运行中 job 和 worker heartbeat records 的间隔。 |
| `WORKER_JOB_MAX_ATTEMPTS` | 是 | 基础设施级 worker 失败的最大尝试次数。文件级处理失败会保留在文件记录上。 |
| `WORKER_JOB_RETRY_DELAY_MS` | 是 | 基础设施级 worker 失败再次尝试前的等待时间。 |
| `WORKER_QUEUE_BACKPRESSURE_LIMIT` | 是 | active source-file worker jobs 达到该数量后，上传 API 返回 `QUEUE_BACKPRESSURE`。 |
| `WORKER_QUEUE_BACKPRESSURE_KB_LIMIT` | 是 | 单个知识库 active source-file worker jobs 达到该数量后，上传 API 返回 `QUEUE_BACKPRESSURE`。 |
| `WORKER_QUEUE_BACKPRESSURE_MAX_AGE_SECONDS` | 是 | 最老排队 job 达到该等待时间后，上传 API 要求客户端减速。 |
| `WORKER_QUEUE_BACKPRESSURE_RETRY_AFTER_SECONDS` | 是 | `QUEUE_BACKPRESSURE` 返回给客户端的建议等待秒数。 |
| `WORKER_SHUTDOWN_GRACE_MS` | 是 | 部署停止 worker 进程时的优雅退出等待时间。 |
| `WORKER_COMPLETED_JOB_RETENTION_DAYS` | 是 | completed worker job history 清理前保留天数。 |
| `WORKER_FAILED_JOB_RETENTION_DAYS` | 是 | failed worker job history 清理前保留天数。 |
| `WORKER_DEAD_LETTER_JOB_RETENTION_DAYS` | 是 | dead-letter worker job history 清理前保留天数。 |
| `WORKER_RETENTION_CLEANUP_BATCH_SIZE` | 是 | 单次 retention cleanup 最多删除的 worker job 行数。 |
...
| `LINK_INDEX_SHARD_SIZE` | 是 | 生成链接索引分片时每个分片的最大条目数。 |
| `MANIFEST_SHARD_SIZE` | 是 | 生成 manifest 分片时每个分片的最大条目数。 |
| `GRAPH_EDGE_SHARD_SIZE` | 是 | 生成图关系分片时每个分片的最大边数量。 |
| `GRAPH_CANDIDATE_LIMIT` | 是 | 单个来源文件生成图关系时最多检查的候选图节点数量。 |
| `GRAPH_MAINTENANCE_BATCH_SIZE` | 是 | 有界图关系刷新任务的最大批量大小。 |
| `ROOT_SUMMARY_LIMIT` | 是 | `index.md`、`log.md` 等根 Markdown 文件中直接展示的来源页面摘要数量上限。完整索引仍通过 `_index` 分片提供。 |

8 核 32 GB 服务器建议从 `DATABASE_POOL_MAX=12`、`WORKER_DATABASE_POOL_MAX=8`、`WORKER_SOURCE_FILE_CONCURRENCY=2`、`WORKER_CLAIM_BATCH_SIZE=10`、`UPLOAD_FILE_PROCESSING_CONCURRENCY=1` 和 `UPLOAD_STORAGE_CONCURRENCY=4` 开始。增加并发前先观察 CPU、内存、S3 吞吐、数据库延迟和模型延迟。PostgreSQL 连接预算按 `api replicas * DATABASE_POOL_MAX + worker replicas * WORKER_DATABASE_POOL_MAX + migration headroom + operational headroom` 计算。
...
文件处理列表展示单个文件的生命周期和生成结果可见性。文件树和 Developer OpenAPI 内容读取始终读取当前 active published release。大型索引超过配置的分片大小后会写成 shard descriptor 和 JSONL shard files，Agent 可以从 `_index/search.json`、`_index/links.json` 和 `_index/manifest.json` 发现 shard paths。根 Markdown 文件通过 `ROOT_SUMMARY_LIMIT` 控制展示规模，完整机器可读数据保留在分片中。
