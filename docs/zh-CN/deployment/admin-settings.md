...
大批量导入期间 CPU、数据库、服务商或存储延迟上升时，优先降低文档并发。
...
| 在途搜索任务数 | 并发等待中的索引任务数。 | 并发批量请求或搜索服务操作数。 | `8` |
...
最终结果 `limit`、`rerankTopK` 和 `rerankScoreThreshold` 属于 Developer OpenAPI 单次请求字段，因此不会出现在 Admin 设置中。`rerank` 默认是 `false`；启用后，接口只发送已经授权的标题、路径和限制长度的来源正文摘要。嵌入余弦相关性阈值仍由生效的嵌入查询策略管理，与重排分数阈值彼此独立。
