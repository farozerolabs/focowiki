...
Developer OpenAPI 让应用可以在不打开 Admin UI 的情况下操作 Focowiki。你可以用它创建知识库、上传 Markdown 文件、观察任务、读取生成文件、删除源文件关联页面、管理 webhooks，并查看 webhook 投递状态。
...
2. 向知识库上传一个或多个 `.md` 文件，并保存 `taskId` 和返回的 `fileId`。
3. 轮询任务详情接口，直到任务 lifecycle 结束。
...
上传一个或多个 Markdown 文件，并保存 `taskId` 和返回的来源文件标识符。把 `guide.md` 和 `faq.md` 替换成本地 `.md` 文件路径。
