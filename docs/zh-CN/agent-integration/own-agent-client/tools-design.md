...
按 `fileId` 调用 `read_file` 时，使用文件树、搜索、文件详情或可见 `generatedFileId` 字段返回的可读文件 ID。读取 `index.md`、`schema.md`、`log.md`、`_graph/index.md`、`_graph/manifest.json`、返回的 `graphRef`、可见 `generatedFilePath` 或从链接发现的页面时，使用逻辑路径。
