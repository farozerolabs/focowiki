...
Use this page when a third-party Agent client can follow Skill instructions and make HTTP requests but cannot register custom tools.

The installable Skill is written for the Agent that answers the end user's question. It exposes only a read-only knowledge endpoint and the evidence workflow. Product implementation, indexing, storage, providers, authentication setup, and upstream response normalization stay in the developer-owned adapter.

## Design Goals

- Use one developer-provided base URL.
...
The developer provides one read-only base URL:

```text
https://knowledge.example.com
```

The adapter may require credentials, but credentials belong in the Agent client's secure configuration and must never be embedded in the Skill.
