...
const INDEX_NAVIGATION_MARKDOWN = new RegExp(
  `^_index/(?:manifest|search|links|tree)/(?:index\\.md|v1/(?:index\\.md|index-${EXTENSION_STABLE_LEAF_ID}\\.md))$`,
  "u"
);
const GRAPH_NAVIGATION_MARKDOWN = new RegExp(
  `^_graph/(?:graph_node|graph_edge)/(?:index\\.md|v1/(?:index\\.md|index-${EXTENSION_STABLE_LEAF_ID}\\.md))$`,
  "u"
);
const BY_FILE_NAVIGATION_MARKDOWN = new RegExp(
  `^_graph/by-file/(?:index\\.md|index-${EXTENSION_STABLE_LEAF_ID}\\.md)$`,
  "u"
);
...
export function isAllowedPublicBundleFilePath(path: string): boolean {
  ...
  if (
    ...
    || BY_FILE_NAVIGATION_MARKDOWN.test(path)
  ) return true;
