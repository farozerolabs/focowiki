...
export type DocumentPublicationItemDelta = Readonly<{
...
export type DocumentPublicationWorkNode = Readonly<{
  identity: string;
...
export function planDocumentPublicationJob(input: Readonly<{
  jobPublicId: string;
  ...
  if (!input.jobPublicId || input.documents.length < 1
  ...
  for (const document of input.documents) {
    ...
    if (document.priorLogicalPath
      ...
      const priorPagePath = pagePath(document.priorLogicalPath);
      deletes.add(priorPagePath);
      deletes.add(portableByFileGraphPath(priorPagePath));
  ...
  addNode(nodes, `validation:${input.jobPublicId}`, "validation",
    input.jobPublicId, [...nodes.keys()]);
  const work = [...nodes.values()].map((node) => ({
    ...node,
...
type MutableNode = {
  identity: string;
  kind: DocumentPublicationWorkNode["kind"];
  key: string;
...
function validateDelta(delta: DocumentPublicationItemDelta): void {
  if (!delta.mutationPublicId || !delta.sourceFilePublicId
    || !delta.sourceRevisionPublicId
    || !Number.isSafeInteger(delta.readinessSequence)
    || delta.readinessSequence < 1
