...
export function createStorageVnextCandidateObjectCleanupWorker(input: {
  ...
  return {
    async runBatch(request: {
      ...
      await input.actions.recoverStale({
        ...
        safeErrorCode: "STALE_CANDIDATE_OBJECT_CLEANUP_LEASE",
        limit: request.limit,
        selector: SELECTOR
      });
        ...
        try {
          ...
          if (hasCode(error, "owners_present")) skippedOwned += 1;
            ...
            await input.actions.releaseForRetry({
              ...
              safeErrorCode: cleanupErrorCode(error),
...
function cleanupErrorCode(error: unknown): string {
  ...
  return "CANDIDATE_OBJECT_PROVIDER_DELETE_FAILED";
}

function workerError(code: string): Error & { code: string } {
  return Object.assign(
    new Error(`Storage vNext candidate object cleanup worker error: ${code}`),
    { code }
  );
}
