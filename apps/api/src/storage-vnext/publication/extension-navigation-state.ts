...
export const STORAGE_VNEXT_EXTENSION_NAVIGATION_SHARD_KIND =
  "extension_navigation";

export function createStorageVnextExtensionNavigationShards(input: {
  ...
  if (!Number.isSafeInteger(input.maximumBytes) || input.maximumBytes < 1) {
    throw extensionStateError("invalid_byte_budget");
  }
  ...
  while (changed) {
    ...
    for (let index = 0; index < groups.length; index += 1) {
      ...
      if (encode(input.directoryPath, index, groups.length, group).byteLength
        <= input.maximumBytes) continue;
      if (group.length < 2) throw extensionStateError("leaf_byte_budget_exceeded");
...
export function parseStorageVnextExtensionNavigationState(input: {
  bytes: Uint8Array;
  directoryPath: string;
}): { partIndex: number; partCount: number; leaves: PersistentDirectoryLeaf[] } {
  assertDirectoryPath(input.directoryPath);
  let value: unknown;
  try {
    value = JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(input.bytes));
  } catch {
    throw extensionStateError("invalid_bytes");
  }
  if (
    !isRecord(value)
    || value.formatVersion !== 1
    || value.kind !== "extension-navigation"
  ...
  ) throw extensionStateError("invalid_document");
  return {
...
function validateLeaves(input: readonly unknown[]): PersistentDirectoryLeaf[] {
  ...
  return input.map((value) => {
    ...
    ) throw extensionStateError("invalid_leaf");
    ...
    const entries = value.entries.map<PersistentDirectoryLeaf["entries"][number]>((entry) => {
      ...
      if (
        ...
        || !entry.targetPath
        || (entry.evidencePath !== undefined
          && (typeof entry.evidencePath !== "string" || !entry.evidencePath))
        || (kind !== "file" && kind !== "directory")
      ) throw extensionStateError("invalid_entry");
...
function assertDirectoryPath(value: string): void {
  if (!value || Buffer.byteLength(value, "utf8") > 4_096) {
    throw extensionStateError("invalid_directory_path");
  }
}
...
function extensionStateError(code: string): Error & { code: string } {
  return Object.assign(
    new Error(`Storage vNext extension navigation state error: ${code}`),
    { code }
  );
}
