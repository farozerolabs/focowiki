...
export class RuntimeSchemaGenerationError extends Error {
  public constructor(public readonly foundGeneration: string | null) {
    super(
      foundGeneration
        ? `Database schema generation ${foundGeneration} is incompatible with ${RUNTIME_SCHEMA_GENERATION}. Start this version with an empty database initialized by the current release.`
        : `Database contains an unmarked or partially initialized Focowiki schema. Start this version with an empty database initialized by the current release.`
    );
