...
export type HardDeleteTargetKind = "source_file" | "knowledge_base";
...
export function createPostgresHardDeleteRepository(sql: DatabaseClient): HardDeleteRepository {
  return {
    ...
    async purgeSourceFileData(input) {
      ...
      await runStage("source_file_search_documents", () =>
        deleteSourceFileSearchDocuments(sql, input, batchSize)
      );
      await runStage("source_file_tree_entries", () =>
        deleteSourceFileTreeEntries(sql, input, batchSize)
      );
    ...
    async purgeKnowledgeBaseData(input) {
      ...
      await runStage("knowledge_base_search_documents", () =>
        deleteKnowledgeBaseSearchDocuments(sql, input, batchSize)
      );
      await runStage("knowledge_base_tree_entries", () =>
        deleteKnowledgeBaseTreeEntries(sql, input, batchSize)
      );
...
async function deleteSourceFileHistoricalReleaseData(
  ...
  return await deleteUntilEmpty(async () => {
    const rows = await sql<CountRow[]>`
      ...
      removed_search AS (
        DELETE FROM focowiki.bundle_file_search_documents document
        USING affected_releases release
        WHERE document.knowledge_base_id = ${input.knowledgeBaseId}
          AND document.release_id = release.release_id
        RETURNING document.bundle_file_id
      ),
      removed_tree AS (
        DELETE FROM focowiki.bundle_tree_entries entry
        USING affected_releases release
      ...
      SELECT
        (
          (SELECT count(*) FROM removed_search)
          + (SELECT count(*) FROM removed_tree)
...
async function deleteSourceFileSearchDocuments(
  sql: DatabaseClient,
  input: SourceFilePurgeInput,
  batchSize: number
): Promise<number> {
  return await deleteUntilEmpty(async () => {
    const rows = await sql<CountRow[]>`
      ...
      removed AS (
        DELETE FROM focowiki.bundle_file_search_documents document
        USING target
        WHERE document.release_id = target.release_id
...
async function deleteSourceFileTreeEntries(
  sql: DatabaseClient,
  input: SourceFilePurgeInput,
  batchSize: number
): Promise<number> {
...
async function deleteRowsBySourceFileId(
  ...
  return deleteSourceFileWorkerJobRows(sql, input);
}
...
async function deleteKnowledgeBaseSearchDocuments(
  sql: DatabaseClient,
  input: KnowledgeBasePurgeInput,
  batchSize: number
): Promise<number> {
...
async function deleteKnowledgeBaseTreeEntries(
  sql: DatabaseClient,
  input: KnowledgeBasePurgeInput,
  batchSize: number
): Promise<number> {
  return deleteKnowledgeBaseIdRows(sql, {
    table: "bundle_tree_entries",
    knowledgeBaseId: input.knowledgeBaseId,
    batchSize
  });
}
...
async function deleteKnowledgeBaseIdRows(
  ...
  if (input.table === "bundle_tree_entries") {
    return deleteKnowledgeBaseTreeEntryRows(sql, input);
  }
...
async function deleteKnowledgeBaseTreeEntryRows(
  sql: DatabaseClient,
  input: { knowledgeBaseId: string; batchSize: number }
): Promise<number> {
  return deleteKnowledgeBaseRowsById(sql, {
    tableName: "bundle_tree_entries",
    alias: "entry",
    knowledgeBaseId: input.knowledgeBaseId,
    batchSize: input.batchSize
  });
...
async function deleteKnowledgeBaseRowsById(
  ...
  if (input.tableName === "bundle_tree_entries") {
    return deleteKnowledgeBaseTreeEntryRowsById(sql, input);
  }
...
async function deleteKnowledgeBaseTreeEntryRowsById(
  sql: DatabaseClient,
  input: { knowledgeBaseId: string; batchSize: number }
): Promise<number> {
  return await deleteUntilEmpty(async () => {
