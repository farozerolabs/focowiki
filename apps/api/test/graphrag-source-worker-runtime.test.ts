...
import {
  ...
  resolvePythonPath
} from "../src/semantic/graphrag/source-worker-runtime.js";
import type { GraphRagPythonProcess } from "../src/semantic/graphrag/python-process.js";

describe("GraphRAG source-worker runtime", () => {
  it("bounds the Python pool by measured source concurrency", () => {
    ...
    expect(resolveGraphRagPoolSize(32)).toBe(2);
  ...
  it("validates Node and adapter paths before semantic claims", async () => {
    ...
    const events: string[] = [];
    const runtime = createGraphRagSourceWorkerRuntime({
