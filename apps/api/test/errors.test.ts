...
describe("redactSecrets", () => {
  it("redacts named secret assignments", () => {
    expect(
      redactSecrets(
        "ADMIN_PASSWORD=admin-secret S3_SECRET_ACCESS_KEY=s3-secret MODEL_API_KEY: model-secret EMBEDDING_API_KEY=embedding-secret rawKey=fwok_secret OPENAPI_KEY=fwok_other"
      )
    ).toBe(
      "ADMIN_PASSWORD=<redacted> S3_SECRET_ACCESS_KEY=<redacted> MODEL_API_KEY: <redacted> EMBEDDING_API_KEY=<redacted> rawKey=<redacted> OPENAPI_KEY=<redacted>"
    );
  });
