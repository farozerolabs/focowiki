...
CREATE TEMPORARY TABLE recoverable_navigation_jobs ON COMMIT DROP AS
SELECT public_id
...
CREATE TEMPORARY TABLE recoverable_navigation_items ON COMMIT DROP AS
SELECT item.public_id, item.knowledge_base_id,
       item.document_job_public_id, item.affected_evidence
FROM recoverable_navigation_jobs recoverable
...
FROM recoverable_navigation_items item
...
USING recoverable_navigation_items item,
...
SET outcome = 'pending', safe_error_code = NULL, terminal_at = NULL,
    updated_at = now()
FROM recoverable_navigation_items recoverable
...
SET generation = 'storage-vnext-v26-navigation-chain-reconciliation'
