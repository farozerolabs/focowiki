CREATE TEMP TABLE focowiki_migration_contention_generations
ON COMMIT DROP
AS
...
WHERE generation.generation_kind = 'normal'
  ...
  AND generation.safe_error_code IN (
    ...
    'PUBLICATION_JOB_DEAD_LETTER'
  )
  AND generation.safe_error_message = 'Immutable object write is already in progress';
...
SET generation = 'immutable-object-contention-recovery-v4'
WHERE singleton = true
  AND generation = 'bounded-publication-recovery-v3';
