CREATE TEMP TABLE focowiki_migration_retry_budget_generations
ON COMMIT DROP
AS
...
WHERE generation.generation_kind = 'normal'
  ...
  AND generation.safe_error_message = 'Projection write will be retried';
...
SET status = 'pending',
    ...
    updated_at = now()
FROM focowiki_migration_retry_budget_generations migrated
...
SET generation = 'publication-retry-budget-recovery-v5'
WHERE singleton = true
  AND generation = 'immutable-object-contention-recovery-v4';
