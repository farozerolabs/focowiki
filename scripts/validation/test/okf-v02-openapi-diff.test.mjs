...
test("classifies the reviewed OKF 0.2 OpenAPI change as additive", () => {
  const diff = createOkfV02OpenApiDiff(document, baseline);

  assert.equal(diff.compatibility, "additive");
  ...
  assert.deepEqual(diff.parameters, {
    added: ["okfStatus", "okfTrustTier", "okfFreshness"],
    removed: []
  });
  assert.deepEqual(diff.responseFields.GeneratedFile.added, ["okfSignals"]);
  assert.deepEqual(diff.responseFields.FileSearchResult.added, ["okfSignals"]);
